...
public class ButterworthLowPassFliterVector3: ButterworthLowPassFliterBase
{
    #region private
    ...
    public Vector3 ButterworthLowpassFliter(Vector3 rawinput, int frequency, int fliterhz)
    {
        ...
        fliter_output = rawinput * fliter_param[0] +  last_input_param_ve3[0] * fliter_param[1] + last_input_param_ve3[1] * fliter_param[2]
                            + last_output_param_ve3[0] * fliter_param[3] +   last_output_param_ve3[1] * fliter_param[4];
        last_input_param_ve3[1] = last_input_param_ve3[0];
        last_input_param_ve3[0] = rawinput;
        last_output_param_ve3[1] = last_output_param_ve3[0];
        last_output_param_ve3[0] = fliter_output;
...
public class ButterworthLowPassFliter: ButterworthLowPassFliterBase
{
    #region private region
    float[] last_input_param = new float[2] { 0.0f, 0.0f };
    float[] last_output_param = new float[2] { 0.0f, 0.0f };

    private float fliter_output = 0;
    #endregion



    public float ButterworthLowpassFliter(float rawinput, int frequency, int fliterhz)
    {
        if (last_fliter_hz != fliterhz)
        {
            if (fliterhz == 20)
...
public abstract class ButterworthLowPassFliterBase
{

    protected float[] fliter_param = new float[5];
    protected int fliter_select;
    protected int last_fliter_hz = 0;


    // 
    protected float[,] butterworth_lowpass_param_1000 = new float[4, 5]{{0.00362f,0.00724f,0.00362f,1.82270f,-0.83720f},   //1000_20hz lowpass fliter
    ...
    protected float[,] butterworth_lowpass_param_800 = new float[4, 5] {{0.00554f,0.01108f,0.00554f,1.77863f,-0.80080f},     //800_20hz lowpass fliter
